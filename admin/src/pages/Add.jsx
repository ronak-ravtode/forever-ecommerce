...
const Add = ({token}) => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col gap-3 items-start w-full'>
        ...
        <button disabled={isSubmitting} className={`w-28 py-3 mt-4 bg-black text-white ${isSubmitting ? 'opacity-60 cursor-not-allowed' : ''}`} type="submit">{isSubmitting ? 'ADDING...' : 'ADD'}</button>
    </form>
  )
}
